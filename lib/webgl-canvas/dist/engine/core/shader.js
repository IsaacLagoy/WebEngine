...
const glConstants_1 = require("../../math/glConstants");
/**
...
function loadProgram(gl, vertex, fragment) {
...
class Shader {
     ...
     */
    static async create(gl, vertexUrl, fragmentUrl) {
    ...
    loadAttributes() {
        ...
        for (let i = 0; i < nAttributes; i++) {
            ...
            const componentCount = (0, glConstants_1.getGLTypeComponentCount)(this.gl, info.type);
            tempAttributes.push({
                name: info.name,
                location: location,
        ...
        for (const attr of tempAttributes) {
            ...
            this.stride += (0, glConstants_1.getGLTypeSize)(this.gl, attr.dataType);
     ...
     */
    bindTextureToSlot(name, texID, target, slot) {
        this.use();
     ...
     */
    bindTexture(name, texture, slot = 0) {
