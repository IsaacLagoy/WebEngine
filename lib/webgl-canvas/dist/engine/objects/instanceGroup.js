 ...
 */
class InstanceGroup {
    constructor(engine, mesh, material) {
        this.nodes = [];
     ...
     */
    setupInstanceAttributes(program) {
        const gl = this.gl();
     ...
     */
    disableInstanceAttributes(program) {
        const gl = this.gl();
     ...
     */
    drawInstanced(gl, program, viewProjMatrix, cameraPos) {
        if (this.nodes.length === 0)
            return;
        ...
        if (this.material) {
            ...
            gl.bindTexture(gl.TEXTURE_2D, material_1.Material.defaultWhiteTexture(gl));
        ...
        gl.drawElementsInstanced(gl.TRIANGLES, this.mesh.numIndices, gl.UNSIGNED_SHORT, 0, this.nodes.length);
