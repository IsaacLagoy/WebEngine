...
import { createLeafNode } from "./leafHelper";
...
export class Tree {
   ...
   */
  private static seededRandom(seedState: { value: number }): number {
  ...
  private static isInFrustum(
    ...
    camNear: number,
    camFar: number,
    frustumWidthNear: number,
    frustumHeightNear: number,
    frustumWidthFar: number,
    frustumHeightFar: number
  ): boolean {
    // Vector from camera to point
    const toPoint = vec3.create();
    vec3.subtract(toPoint, worldPos, camPos);

    // Project onto forward vector to get distance along view direction
    const distance = vec3.dot(toPoint, forward);
    ...
    const rightDist = vec3.dot(toPoint, right);
    const upDist = vec3.dot(toPoint, up);
    ...
    return (
      Math.abs(rightDist) <= frustumWidth * (0.5 + margin) * spawnWidthMultiplier &&
   ...
   */
  static async spawnTrees(
    engine: Engine,
    scene: Scene,
    treeMesh: Mesh,
    barkMaterial: Material,
    config: TreeSpawnConfig
  ): Promise<{ treeNodes: Node[]; leaves: Node[] }> {
    ...
    vec3.cross(right, forward, scene.camera.up);
    ...
    while (treesPlaced < config.numTrees && attempts < maxAttempts) {
      ...
      let logX: number = 0;
      let logZ: number = 0;
      let candidatePos: vec3 | null = null;
      ...
      for (let tryCount = 0; tryCount < 50; tryCount++) {
        ...
        vec3.scale(
          rightOffset,
          right,
          (seededRandom() - 0.5) * frustumWidthAtDist * 0.9
        );
        const upOffset = vec3.create();
        vec3.scale(
          upOffset,
          up,
          (seededRandom() - 0.5) * frustumHeightAtDist * 0.9
        );

        vec3.add(localOffset, forwardOffset, rightOffset);
        vec3.add(localOffset, localOffset, upOffset);

        // Convert to world position
        const testPos = vec3.create();
        vec3.add(testPos, camPos, localOffset);
      ...
      const distanceFromFire = Math.sqrt(
        (logX - config.fireCenter[0]) ** 2 + (logZ - config.fireCenter[2]) ** 2
      );
      ...
      const distanceFromCamera = Math.sqrt(
        (logX - camPos[0]) ** 2 + (logZ - camPos[2]) ** 2
      );
      ...
      const logHeight =
        (config.logBaseScale + seededRandom() * config.logHeightVariation) * sizeMultiplier;
    ...
    console.log(
      `Added ${treesPlaced} vertical logs (trees) across terrain (${attempts} attempts)`
    );

    // Place leaves on trees
    const leaves = await Tree.addLeavesToTrees(
      engine,
      scene,
      treeNodes,
      treeVertices,
   ...
   */
  static async addLeavesToTrees(
    engine: Engine,
    scene: Scene,
    treeNodes: Node[],
    treeVertices: [number, number, number][],
    config: TreeSpawnConfig
  ): Promise<Node[]> {
    // Load leaf resources
    const quadMesh = await Mesh.fromObj(engine, "/models/quad.obj");
    const leafMaterial = new Material(engine);
    ...
    for (const treeNode of treeNodes) {
      ...
      for (const [localX, localY, localZ] of treeVertices) {
        ...
        const localVertex = vec3.fromValues(localX, localY, localZ);
        const worldVertex = vec3.create();

        // Transform the vertex using the tree's model matrix
        vec3.transformMat4(worldVertex, localVertex, treeNode.modelMatrix);

        // Calculate distance from camera (only X and Z, ignoring Y height)
        const distanceFromCamera = Math.sqrt(
          (worldVertex[0] - camPos[0]) ** 2 + (worldVertex[2] - camPos[2]) ** 2
        );
        ...
        const leaf = createLeafNode(
          scene,
          worldVertex,
          vec3.fromValues(leafSize, leafSize, 1),
          quadMesh,
          leafMaterial
        );
    ...
    console.log(
      `Added ${totalLeaves} leaves at tree vertices (${treeNodes.length} trees × ${treeVertices.length} vertices each)`
    );
