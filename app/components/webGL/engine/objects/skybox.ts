...
export class Skybox {
    ...
    sunElevation: number = 0.3;
    sunAzimuth: number = 0.0;
    turbidity: number = 2.5;
    sunIntensity: number = 1.0;
    
    private sunDirection: vec3 = vec3.create();
    private cycleTime: number = 0.0; // Time parameter for day-night cycle (0.0 to 1.0)
    ...
    constructor(engine: Engine, scene: Scene, mesh: Mesh, shader: Shader) {
        ...
        mat4.fromScaling(this.modelMatrix, vec3.fromValues(this.size, this.size, this.size));
    ...
    getLightDirection(): vec3 | null {
        ...
        if (this.sunDirection[1] > -0.1) {
            ...
            vec3.negate(moonDir, this.sunDirection);
            moonDir[1] = Math.abs(moonDir[1]); // Ensure moon is above horizon
            vec3.normalize(moonDir, moonDir);
            return moonDir;
        }
    }
