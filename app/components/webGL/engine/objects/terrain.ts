...
export class Terrain {
    ...
    private static accumulateNormal(
        ...
        accumulator: { x: number; y: number; z: number; count: number }
    ): void {
        if (normal) {
    ...
    private static calculateNormal(
        ...
        if (x < segmentsX && z < segmentsZ) {
            ...
            Terrain.accumulateNormal(
                Terrain.calcTriangleNormal(current, bottom, right),
                accumulator
            );
        }
        ...
        if (x > 0 && z < segmentsZ) {
            ...
            const bottom = getVertex(x, z + 1);      // c
            Terrain.accumulateNormal(
                Terrain.calcTriangleNormal(left, bottom, current),
                accumulator
            );
        }
        ...
        if (x > 0 && z < segmentsZ) {
            ...
            Terrain.accumulateNormal(
                Terrain.calcTriangleNormal(current, bottom, bottomLeft),
                accumulator
            );
        }
        ...
        if (x < segmentsX && z > 0) {
            ...
            Terrain.accumulateNormal(
                Terrain.calcTriangleNormal(topRight, current, right),
                accumulator
            );
        }
     ...
     */
    private static readonly defaultParams: Required<Omit<TerrainParams, 'maxDistance'>> & { maxDistance: number | null } = {
        width: 1,
        height: 1,
        segmentsX: 1,
        segmentsZ: 1,
    ...
    static generatePlane(params: TerrainParams = {}): {
        ...
        const {
            ...
            segmentsX = Terrain.defaultParams.segmentsX,
            segmentsZ = Terrain.defaultParams.segmentsZ,
